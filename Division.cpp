...
*Description: division class definitions for constructor, destructor, print and evaluate.
*/
#include <sstream>
#include "Division.h"
using namespace std;
...
Division::Division(string ls, string rs, string branch):
...
string Division::evaluate() {
	...
	if (this->branch == "bothleaf") {//if the node branches to two numbers.
		...
		return tostring(convert(left->evaluate()) / convert(right->evaluate()));//if both direct branches are not numbers, return the string containing the result of left evaluate() / right evaluate()
